...
module.exports = (wst_server = class wst_server {
  ...
  constructor(sharedSecret, dstHost, dstPort, webapp) {
    ...
    try {
      ...
      if (!uri.query.tkn) {
          ...
          if(!this.sharedSecret) {
              ...
              let csig = crypto.createHmac("sha256", this.sharedSecret).update(parts[0]+':'+parts[1]).digest("hex");
              return csig === parts[2].toString() ? {host, port} : undefined;
          }
      }
    }catch {
      return undefined;
