...
const httpSetup = require("../httpSetup");
const debug = require("../debug");

const { BlockSize, AutoSealDelayMs, ReqOpenIntervalMs } = require("./Constants");
...
if (debug.isDebug) {
  dlogSock = (msg, socket) => log(`[${socket && socket.address() ? socket.address().port : ''}]${msg}`);
} else {
  dlogSock = function() {};
}
...
module.exports = class ClientConn extends require("stream").Duplex {
  ...
  constructor(urlEndPoint) {
    ...
    if (this.url.protocol === 'https:') {
      this.doHttp = https;
      if (!this.url.port) { this.url.port = 443; }
      this.sendAgent = httpSetup.createHttpsAgent({maxSockets: 1, keepAlive: true, keepAliveMsecs: 10000});
      this.recvAgent = httpSetup.createHttpsAgent({maxSockets: 1, keepAlive: true, keepAliveMsecs: 10000});
    } else {
      this.doHttp = http;
      if (!this.url.port) { this.url.port = 80; }
      this.sendAgent = new http.Agent({maxSockets: 1, keepAlive: true, keepAliveMsecs: 10000});
      this.recvAgent = new http.Agent({maxSockets: 1, keepAlive: true, keepAliveMsecs: 10000});
    }
  }

  // headers is optional
  connect(...args1) {
    const args = args1.slice(0, args1.length - 1), cb = args1[args1.length - 1];
    let [headers] = Array.from(args);
    ...
    req.on('response', res => {
      ...
      if (res.statusCode === 200) {
        ...
        return cb(new Error(`Http conn rejected, status: ${res.statusCode}, msg: ${res.headers['x-htunrejectmsg'] || res.headers['x-websocket-reject-reason'] || ''}`));
      }
    });
    req.end();
  ...
  _recv() {
    ...
    req.on('error', e => this.emit('error', new Error(`http recv ${e.toString()}`)));
    ...
    req.on('response', res => {
      ...
      if (res.statusCode !== 200) {
        this.emit('error', new Error(`Http recv error, status: ${res.statusCode}`));
      }
      let n = 0;
      res.on('data', d => {
        this.push(d);
      ...
      return res.on('end', () => {
        if (n > 0) { dlogSock(`Cli recv ${n}`, res.socket); }
        if (!this._ended) { return this._recv(); }
      });
    });   // start next request
  ...
  _makeSendReq() {
    ...
    req.on('error', e => this.emit('error', new Error(`http send ${e.toString()}`)));
    req.on('socket', sock => sock.setNoDelay(true));
    req.setTimeout(ReqOpenIntervalMs * 2, () => req.abort()); // triggers error event
    ...
    req.seal = cb => {
      if (req._sealed) { return; }
      req._sealed = true;
      req.on('response', res => {
        res.on('error', e => this.emit('error', e));
        if (res.statusCode !== 200) {
          this.emit('error', new Error(`Http send error, status: ${res.statusCode}`));
        }
        res.on('data', d => {});
        return res.on('end', () => {
          ...
          if (!this._ended) {
            this._makeSendReq();
            if (this._chunk) {  // while sealing, another _write is called
