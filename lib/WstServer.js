...
module.exports = (wst_server = class wst_server {
  ...
  constructor(sharedSecret, uid_header, protocol, dstHost, dstPort, webapp) {
    this.dstHost = dstHost;
    this.dstPort = dstPort;
    this.protocol = protocol;
    this.sharedSecret = sharedSecret;
    this.uid_header = uid_header.toLowerCase();
      ...
      const handleReq = (request, connWrapperCb) => {
        ...
        return this.authenticate(httpRequest, (rejectReason, target, monitor) => {
          ...
          var tcpConn = net.connect({host, port, allowHalfOpen: false}, () => {
            ...
            let wsConn = request.accept(this.protocol, request.origin);
  ...
  authenticate(httpRequest, authCb) {
    ...
    if (this.dstHost && this.dstPort) {
      ...
      return this.parseUrlDst(httpRequest.url, httpRequest.headers, function(dst) {
          if (!dst) {
  ...
  parseUrlDst(requrl, reqheaders, cb) {
    try {
      ...
      if (!uri.query.tkn) {
          ...
          if(!this.sharedSecret) {
              ...
              dns.lookup(parts[0], function(err, addr) {
                  if(err)
                      return cb(undefined);
                  const [host, port] = [addr, parts[1]];
                  cb(crypto.timingSafeEqual(csig, Buffer.from(parts[2], 'hex')) ? {host, port} : undefined);
              });
          }
      }
    }catch(e) {
